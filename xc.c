...
static PyObject *pyxc_domain_unpause(XcObject *self, PyObject *args)
{
	...
	return xcop(self, args, xc_domain_unpause); //xc for domain group unpause
}
...
static PyObject *pyxc_domain_destroy(XcObject *self, PyObject *args)
{
	...
	return xcop(self, args, xc_domain_destroy);  //xc for domain group destroy
}
...
****************************************/
        ...
        for ( j = 0; j < sizeof(xen_domain_handle_t); j++ )	// iterate over group domain item list
...
****************************************/
	for (j = 0; j < sizeof(xen_domain_group_handle_t); j++)
	    PyList_SetItem(pydg_handle, j, PyInt_FromLong(info[i].dg_handle[j]));		

        PyDict_SetItemString(info_dict, "handle", pyhandle);
		PyDict_SetItemString(info_dict, "dg_handle", pydg_handle);		/* Py group handler*/
        Py_DECREF(pyhandle);
                                     ...
                                     PyObject *kwds)
{
	...
	/* put domain group info into nr_grps by xc_domain_group.c  */
    nr_grps = xc_domain_group_getinfo(self->xc_handle, first_grp, max_grps, info);
    ...
    /* iterate the returned groups(nr_grps) and 
                                    ...
                                    PyObject *kwds)
{
    ...
    if ( pyhandle != NULL )
    {
		...
		/* put xen_domain_group_handle_t into pyhandle */
